...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    score = np.dot(outsideVectors, centerWordVec)#每个外部词的概率都会计算
    y_h = softmax(score)#每一位代表那个词的概率
    loss = -np.log(y_h[outsideWordIdx])#outsideWordIndex外部词在U中的位置
    y = np.eye(y_h.shape[0])[outsideWordIdx]#真实值
